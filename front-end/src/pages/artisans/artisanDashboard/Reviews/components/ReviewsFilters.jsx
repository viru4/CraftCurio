...
const ReviewsFilters = ({ filters, setFilters, stats }) => {
    ...
    <div className="bg-white rounded-xl shadow-sm border border-[#e8d5c4] p-4 sm:p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <option value="5">5 Stars ({stats.byRating['5']})</option>
          <option value="4">4 Stars ({stats.byRating['4']})</option>
          <option value="3">3 Stars ({stats.byRating['3']})</option>
          <option value="2">2 Stars ({stats.byRating['2']})</option>
          <option value="1">1 Star ({stats.byRating['1']})</option>
        </select>

        {/* Reply Status Filter */}
        <select
          value={filters.hasReply}
          onChange={(e) => setFilters({ ...filters, hasReply: e.target.value, page: 1 })}
          className="px-4 py-2.5 border border-[#e8d5c4] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#ec6d13] focus:border-transparent text-sm"
        >
