...
sequenceDiagram
  ...
  API-->>WS: Bid Accepted
  WS-->>FE: emit("auction:newBid", updatedData)
  WS-->>OtherClients: broadcast("auction:newBid", updatedData)
```

### 5.4 Chat & Messaging
Enables real-time private messaging between users.

*   **Features:** Start Conversation, Send Message, Typing Indicators, Read Receipts.
*   **Implementation:** Uses `chatSocket` namespace.

### 5.5 Admin Dashboard & CMS
Provides tools for platform administration and content management.

*   **Features:** Manage Users, Edit "About Us" Content (Mission, Vision, Gallery), View System Stats.

## 6. Database Design

The database uses MongoDB with Mongoose schemas to model the application data.

### 6.1 ER Diagram

```mermaid
erDiagram
    USER ||--o{ ARTISAN_PRODUCT : creates
    ...
    ARTISAN ||--o{ ARTISAN_PRODUCT : crafts
    
    COLLECTIBLE ||--o{ AUCTION_BID : receives
    USER ||--o{ AUCTION_BID : places
...
*   **ArtisanProducts:** Stores details of products listed by artisans.
*   **Collectibles:** Stores items for sale/auction by collectors.
*   **Conversations/Messages:** Stores chat history.
*   **AboutUs:** Stores dynamic content for the About Us page.

## 7. Detailed Design & Implementation

### 7.1 Backend Implementation
The backend is structured using a modular approach:
*   **Routes:** Define API endpoints (e.g., `/api/auth`, `/api/products`).
*   **Controllers:** Handle business logic (e.g., `authController.js`, `productController.js`).
*   **Models:** Mongoose schemas for data validation.
*   **Sockets:** `auctionSocket.js` and `chatSocket.js` handle real-time events.

**Example: Auction Socket Logic**
The `auctionSocket.js` listens for `joinAuction` events to subscribe users to specific auction rooms. When a `placeBid` event is received, it validates the bid against the database, updates the current bid, and broadcasts the new state to all users in the room.
